...
const metadata = { name: "starlight-theme-release-probe", version: "0.3.0" };

describe("0.3.0 release producer provenance", () => {
  it("reports the current release producer for Theme v1 while preserving its CSS", () => {
    ...
    expect(compiled.descriptor.provenance.compilerVersion).toBe("0.3.0");
  ...
  it("binds current producer metadata and verifies a fresh packet without rewriting history", () => {
    ...
    const probe = `
      ...
      const packet=createThemeCatalogCandidate(record.spec,{metadata:{name:'starlight-theme-release-probe',version:'0.3.0'},accent:record.accent});
