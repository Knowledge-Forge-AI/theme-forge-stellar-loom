...
async function run() {
  ...
  try {
    ...
    if (installedMetadata.version !== "0.3.0" || Object.keys(installedMetadata.dependencies ?? {}).length) throw new Error("Unexpected package version or runtime dependencies");
    ...
    if (!versionOutput.includes("0.3.0")) {
      throw new Error(`Unexpected CLI version: '${versionOutput}', expected to contain '0.3.0'`);
    }
    ...
    if (descriptorContent.provenance.compilerVersion !== "0.3.0") {
    ...
    if (genProvenance.generatorVersion !== "0.3.0") {
      throw new Error(`Generated package provenance generatorVersion mismatch: ${genProvenance.generatorVersion}`);
    }
    console.log(`   CLI generate output verified (${parsedGen.filesWritten.length} files, private: true, generatorVersion: 0.3.0)`);
    ...
    const esmTestScript = `
      ...
      if (COMPILER_VERSION !== "0.3.0") throw new Error("Unexpected COMPILER_VERSION: " + COMPILER_VERSION);
